...
// PVトラッキング（LPのindex.htmlから呼び出す）
export async function POST(req: NextRequest) {
  const { path, referrer, sessionId } = await req.json()

  const ip = req.headers.get('x-forwarded-for')?.split(',')[0] || ''
  const ipHash = createHash('sha256').update(ip + process.env.ANALYTICS_SALT!).digest('hex').slice(0, 16)
...
export async function GET(req: NextRequest) {
  ...
  if (!daily) return NextResponse.json({ error: 'no data' }, { status: 500 })
  ...
  return NextResponse.json({
    summary: {
      totalViews: daily.length,
      totalSessions: new Set(daily.map(r => r.session_id)).size,
      totalVisitors: new Set(daily.map(r => r.ip_hash)).size,
      leads: leadCount || 0,
      chats: chatCount || 0,
    },
    daily: dailyStats,
